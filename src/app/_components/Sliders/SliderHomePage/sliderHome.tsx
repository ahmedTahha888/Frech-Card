    ...
    <div className="w-full ">
      ...
      <div className="w-full relative">
        <Swiper
          ...
          },bulletActiveClass:'bg-white! w-[34px]! h-[12px]! opacity-100! rounded-xl! transition-all! duration-500! hover:scale-105!'
        }}     
          spaceBetween={0}
          slidesPerView={1}
          loop={true}
        ...
        >
            ...
            <SwiperSlide key={id}>
              ...
              <Image src={slide.img} alt="Featured Slider" width={200} height={200} className="w-full h-[400px] object-cover"/>
                <div className=" absolute inset-0  bg-linear-to-r from-green-500/90 to-green-400/50">
                  <div className=" h-full w-[90%] flex flex-col justify-center mx-auto">
                      <h3 className="text-white text-3xl font-bold mb-4 max-w-96">{slide.title}</h3>
                      <p className="text-white font-normal">{slide.content}</p>
                      <div className="flex gap-2.5 mt-4">
                          <button className={`${slide.btnColor} font-semibold bg-white py-2 px-6 rounded-lg hover:scale-105 transition-transform`}>{slide.btnText1}</button>
                          <button className='text-white border-2 font-semibold border-white/50 py-2 px-6 rounded-lg hover:scale-105 transition-transform'>{slide.btnText2}</button>
                      </div>
                  </div>
                </div>
          </SwiperSlide>
